...
public class Login extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        txt_SignUp_CustomerId = new javax.swing.JTextField();
        txt_SignUp_Phone = new javax.swing.JFormattedTextField();
        ...
        txt_SignUp_CustomerId.setEditable(false);
        gridBagConstraints = new java.awt.GridBagConstraints();
        gridBagConstraints.gridx = 2;
        gridBagConstraints.gridy = 5;
        gridBagConstraints.fill = java.awt.GridBagConstraints.HORIZONTAL;
        ...
        gridBagConstraints.weightx = 0.1;
        gridBagConstraints.insets = new java.awt.Insets(0, 0, 5, 21);
        jPanel7.add(txt_SignUp_CustomerId, gridBagConstraints);

        txt_SignUp_Phone.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyReleased(java.awt.event.KeyEvent evt) {
                txt_SignUp_PhoneKeyReleased(evt);
            }
        });
        gridBagConstraints = new java.awt.GridBagConstraints();
        gridBagConstraints.gridx = 2;
        ...
        gridBagConstraints.anchor = java.awt.GridBagConstraints.LINE_START;
        gridBagConstraints.insets = new java.awt.Insets(0, 0, 5, 21);
        jPanel7.add(txt_SignUp_Phone, gridBagConstraints);
    ...
    private void lbl_Login_formMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_lbl_Login_formMouseClicked
        ...
        levanan.clearData(txt_SignUp_Name, txt_SignUp_Address, txt_SignUp_Phone, txt_SignUp_CustomerId);
    ...
    private void btn_SignUpKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_btn_SignUpKeyReleased
        if(evt.getKeyCode()==KeyEvent.VK_UP){
            txt_SignUp_Phone.requestFocus();
        }else if(evt.getKeyCode()==KeyEvent.VK_ENTER){
            btn_SignUpActionPerformed(null);
        }
        
    }//GEN-LAST:event_btn_SignUpKeyReleased
    private void txt_SignUp_PhoneKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txt_SignUp_PhoneKeyReleased
        levanan.moreSmooth(txt_SignUp_Address, btn_SignUp, evt);
    }//GEN-LAST:event_txt_SignUp_PhoneKeyReleased
    private void txt_SignUp_AddressKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txt_SignUp_AddressKeyReleased
        levanan.moreSmooth(txt_SignUp_Name, txt_SignUp_Phone, evt);
    ...
    private void txt_Login_UserNameKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txt_Login_UserNameKeyReleased
        levanan.moreSmooth(txt_Login_UserName, txt_Login_UserPassword, evt);
    ...
    private void txt_SignUp_NameKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txt_SignUp_NameKeyReleased
        levanan.moreSmooth(txt_SignUp_Name, txt_SignUp_Address, evt);
    ...
    // Lấy dữ liệu từ các trường nhập liệu
            String customerName = txt_SignUp_Name.getText().trim();
            String customerAddress = txt_SignUp_Address.getText().trim();
            String customerPhone = txt_SignUp_Phone.getText().trim();
            ...
            // Kiểm tra độ dài và định dạng số điện thoại (ví dụ: 10 chữ số)
            if (!customerPhone.matches("\\d{10}")) {
                JOptionPane.showMessageDialog(null, "Số điện thoại không hợp lệ. Vui lòng nhập đúng 10 chữ số.", "Lỗi", JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            // Tạo một customerId mới và kiểm tra trùng lặp
            do {
                int randomNum = random.nextInt(1000);
                customerId = "KH" + String.format("%03d", randomNum);
                isUnique = true;

                // Kiểm tra trùng lặp
                for (Customer customer : customersList) {
                    if (customer.getCustomerId().equals(customerId) || customer.getPhone().equals(customerPhone)) {
                        isUnique = false;
                        break;
                    }
                }
            ...
            // Cập nhật txt_SignUp_CustomerId với customerId hợp lệ
            txt_SignUp_CustomerId.setText(customerId);
            ...
            // Chèn đối tượng Customer vào cơ sở dữ liệu
            try {
                customerDAO.insertCustomerToDB(customer);
                JOptionPane.showMessageDialog(null, "Thêm khách hàng thành công", "Chúc mừng", JOptionPane.INFORMATION_MESSAGE);
            } catch (Exception e) {
                JOptionPane.showMessageDialog(null, "Đã xảy ra lỗi khi thêm khách hàng", "Lỗi", JOptionPane.ERROR_MESSAGE);
                e.printStackTrace();
            }
    }
    public void checkAccount(){
        List<Account> accountList = accountDAO.loadAllAccountsData();
        String username = txt_Login_UserName.getText().trim(); // Lấy giá trị từ JTextField
        String password = new String(txt_Login_UserPassword.getPassword()).trim(); // Lấy giá trị từ JPasswordField

        boolean isAuthenticated = false; // Cờ để xác thực

        for (Account account : accountList) {
            if (account.getUsername().equals(username) && account.getPass().equals(password)) {
        ...
        if (isAuthenticated) {
            ...
            JOptionPane.showMessageDialog(null, "Sai thông tin đăng nhập", "Cảnh báo", JOptionPane.WARNING_MESSAGE);
        }
    ...
    private javax.swing.JTextField txt_SignUp_CustomerId;
    private javax.swing.JTextField txt_SignUp_Name;
    private javax.swing.JFormattedTextField txt_SignUp_Phone;
