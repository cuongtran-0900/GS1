...
public class LeVanAn {
    ...
    // 2. Xóa dữ liệu hiện tại ( instanceof dùng để kiểm tra Object... có nằm trong Class ... hay không )
    public void clearData(JComponent... components) { 
        for(JComponent component : components ){
            if (component instanceof JRadioButton) {
                ((JRadioButton) component).setSelected(false);  // Nếu là JRadioButton, đặt về false
            }else if (component instanceof JTextField) {
                ((JTextField) component).setText("");           // Nếu là JTextField, đặt text thành chuỗi rỗng
            }else if (component instanceof JTextArea) {
                ((JTextArea) component).setText("");            // Nếu là JTextArea, đặt text area thành chuỗi rỗng
            }
        }
    }
